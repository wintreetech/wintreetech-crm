...
import { FolderOpen, Pen, Plus, Search, Trash } from "lucide-react";
...
function DevelopmentDashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const expiredCount = records.filter((item) => {
      ...
      return new Date(item.expiredOn) < today;
    }).length;

    const dueSoonCount = records.filter((item) => {
      if (!item.expiredOn) return false;
  ...
  const handleCreate = async (payload) => {
    try {
      const { message } = await dispatch(createDevelopmentRecord(payload)).unwrap();
    ...
    <div className="p-4 bg-gray-50 dark:bg-gray-800 min-h-screen">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6 text-black">
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm dark:bg-gray-900 dark:text-white">
          ...
          <p className="text-gray-400 text-xs">Records with a website present</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-4 dark:bg-gray-900">
        ...
        <div className="overflow-x-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-100 rounded-lg overflow-y-visible border border-gray-200 dark:border-gray-700">
          <table className="min-w-max w-full text-left text-xs sm:text-sm text-black dark:text-white">
            ...
            <tbody>
              {loading ? (
                <TableLoader columns={9} message="Loading development records..." />
                ...
                currentRecords.map((record) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4">{record.registrarPlatform || "-"}</td>
                    <td className="py-3 px-4">{record.companyDirector || "-"}</td>
                    <td className="py-3 px-4">{record.merchantCountry || "-"}</td>
                    <td className="py-3 px-4">{formatDate(record.expiredOn)}</td>
                    <td className="py-3 px-4">
                      <button
                        type="button"
                    ...
                    <td className="py-3 px-4">{formatDate(record.createdAt)}</td>
                      ...
                      <div className="flex justify-center gap-3">
                        <button
                          ...
                          className={`text-blue-600 cursor-pointer hover:text-blue-800 ${
                            !hasPermission ? "opacity-40 pointer-events-none" : ""
                          }`}
